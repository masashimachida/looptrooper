...
**自律的なコード保守ループの基盤。**
`loop` ラベルを付けた GitHub issue を入力に、無人のエージェントが worktree 隔離から実装、検証、PR 作成までを行う。
マージは人間が行う。

このドキュメントは LoopTrooper をゼロから動かすための手順書である。
設計の背景、仕組み、用語は [`doc/`](./doc/) を参照。

- [doc/architecture.md](./doc/architecture.md)：なぜこの形か（外部トリガ駆動、五つの罠、3層構造、dind、コスト）
- [doc/mechanism.md](./doc/mechanism.md)：タスクのライフサイクルとトリガ仕様（issue/PR/deps/ci/spec/outcome、依存関係、redo/long、メモリ）
- [doc/glossary.md](./doc/glossary.md)：用語集（ドライバ、番人、Fixer/Verifier、dind などの固有語）
...
- **Claude のサブスク（Pro または Max）**。API キーは不要で、対話セッションを使う
...
App を作らない場合は、対象 repo 一個に絞った fine-grained PAT（上表と同じ権限）を発行し、`GH_TOKEN` に入れる。
PAT は App が未設定のときだけ使われる。

---

## ステップ3：起動する（`loopctl` 推奨）

複数 project を1台でまとめて操作する薄い CLI `bin/loopctl` を使う。
`project = 1 コンテナ` のまま、`init → create → start → login` で増減できる。
...
```
> 設定の優先順位は **`.env` > `loop.yaml` > 既定（`config.sh`）**。
> 秘密は `.env` に、それ以外は `loop.yaml` に書く。
...
> **構成**：`~/.looptrooper/`（`.env` 共有1枚と `projects/<name>/{config/loop.yaml, .loop}`）。`LOOPCTL_HOME` で場所を変更できる。
...
タスク入力は `loop` ラベル付き issue に限定する。
対象 repo に issue を立てて流す:
...
次の poll でループが拾う。
PR が立てば完了で、`loopctl status myapp` や通知に「PR ready」が出る。
進行はログで追う:
...
まず一括ダッシュボードを見れば、番人、認証、タスク、GitHub、メモリ、アウトカム、直近通知が一目で分かる:
...
| 長時間タスクを許可 | issue に **`loop:long` ラベル**（チェックポイントを20分から60分に延長） |
