...
> 出典思想: Addy Osmani "Loop Engineering"。詳細は `README.md` / `USAGE.md`。
...
## 「箱」と「本体」の分離（最初に理解すべき構造）

- **ルート** = 「箱」: `Dockerfile` / `docker-compose.yml` / `.env*` / ドキュメント。
- **`app/`** = 「本体」: ループが実際に動かすスクリプト・設定・状態。
- `Dockerfile` は `COPY app/ /work/loop` で **app/ の中身だけ**をイメージに入れる。
  → コンテナ内では `app/bin/driver.sh` は `/work/loop/bin/driver.sh`、`LOOP_DIR=/work/loop`。
  パスを追うときは「`app/` を剥がすと `/work/loop/`」と読み替える。
...
3. **検証は自己採点禁止（Fixer/Verifier 分離）。** タスク処理手順は `app/.claude/skills/loop-task/SKILL.md`。
...
cat app/LOOP_STATE.md                                 # 人間用の状態ボード（静的メモ。ライブ状況は status.sh）
tail -f app/.loop/logs/driver.log                     # 判断系ログ
tail -f app/.loop/logs/session.log                    # claude 生ストリーム（forensics）
tail -f app/.loop/logs/poller.log                     # issue ポーリング

# スクリプトの構文チェック（このリポジトリにテストスイートは無い ── 全て bash）
bash -n app/bin/*.sh app/bin/loop-report app/triggers/*.sh
```
...
- **設定の入力は2層**: `config.sh` 冒頭が `loop.yaml`（箱レベルにマウント・非秘密設定・`yq` でパース）を読み、環境変数へ展開する。**優先順位は env（`.env`/compose）> `loop.yaml` > 既定**（`config.sh` の `${VAR:-...}`）。**秘密（GitHub App 秘密鍵 / `GH_TOKEN` / `SLACK_WEBHOOK_URL` / `NOTIFY_CMD` / `TZ`）は `.env` のまま**、それ以外の設定は `loop.yaml`（雛形 `loop.yaml.example`）。yaml はフラットな「環境変数名: 値」形式で、新キーを足しても loader 改修は不要。`loop.yaml` が無い/`yq` 不在ならフェイルセーフで `.env` だけで動く。session-keeper が config を source して tmux を起こすので claude セッションにも継承される。
