...
5. **権限の最小化。** コンテナに渡す秘密は1個だけ（**GitHub App の秘密鍵**、PAT 運用なら `GH_TOKEN`）。
   ...
   **秘密は claude セッションの env に置かない**: `.env` の秘密（App 鍵/`GH_TOKEN`/webhook＝`LOOP_SECRET_VARS`）は
   session-keeper が tmux 起動時に環境から抜く（deny の `Read(.env)` は env を守れない＝printenv 対策）。
   claude 側の gh はトークンキャッシュ（`.loop/.gh-token`）を読むだけで、鮮度は鍵を持つ poller が毎 tick 温める。
   `container/app/.claude/settings.json` は
   `acceptEdits` ＋ allowlist ＋ deny（main 直 push / force / `--no-verify` / `gh pr merge` / `rm -rf` / `.env` 読み / `.claude/` 編集を禁止）。
   push は `loop/<id>` ブランチのみ、マージは必ず人間。PR の向き先(base)は `PR_BASE_BRANCH`（既定 main）で設定可＝作業ブランチもこの base から切る。
   **push 先の強制は pre-push hook で構造化**（`setup-target` が対象 repo に設置）: settings.json の deny は文字列前方一致で `git push origin loop/x:main` 等の refspec 迂回を素通しするため、push される remote ref を見て `refs/heads/loop/*` 以外とブランチ削除を拒否する（refspec の書き方によらず宛先だけで判定）。`--no-verify` の飛ばしは deny で入口を塞ぐ。main の実保護はプロジェクト任意の branch protection（この基盤は要求しない＝保護するかは repo 次第）。
...
- **claude 生存とデッドタイム短縮**: liveness 判定は `lib.sh` の `claude_alive()`（pane 前面が claude/node か）に集約（keeper と driver で共有）。driver は注入前に `wait_claude_alive`（`CLAUDE_WAIT`＝既定60s）で claude の復帰を待ち、死んだ pane（bash）へ `/clear` やタスクを打ち込む事故を防ぐ。待機は `wait_result_alive` で、claude が `CRASH_DETECT_GRACE`（既定10s）連続で死んでいたら満了（最大 TASK_TIMEOUT）を待たず早期に再キューする（`requeue_crashed` に一本化）。立て直し済みで idle に戻った曖昧ケースは従来どおり満了→`classify_stuck`(hung) に委ねる（誤検知回避）。
- **worktree 残骸の掃除（無限クラッシュループ防止）**: 中断タスクは `loop/<id>` worktree/ブランチ（PR レビューは `pr_branch`）を残す。同一 id で再キューすると `git worktree add` が「already exists / already checked out」で決定的に失敗し、LLM が復旧にもがいて時間を溶かし→また殺される無限ループに陥る。driver が `clean_stale_worktree()` で構造的に断つ（LLM の対処運に依存しない＝番人は素の bash）。**requeue 安全性は `process_one` の注入前 1 点に集約**（crash/hung/limit/中断無応答/`recover_inflight` の全再キュー経路をここでカバー）。加えて worktree を作った終端（done/skipped/failed/blocked/timeout/modal）でもリーク防止に掃除する。掃除対象ブランチはタスク本文から解決（通常＝`loop/<id>`、PR レビュー＝`pr_branch`。後者は remote 追跡ブランチなので worktree を外すだけでブランチは消さない）。SKILL 側も worktree 操作を repo ルート起点に固定し（前回試行の worktree が消えて cwd が無効なことがある）冪等 add する二段構え。
...
- **`planner/`**（root の人間レイヤ）= **人間が対象 repo の checkout で使う**プランナー道具（`commands/loop-issue.md`・`commands/loop-spec.md`）。コンテナにも対象 repo にも自動配布されない＝人間が `~/.claude/` へコピーして使う。**`container/app/`（イメージ payload）の外**に置く（配布されない＝コンテナに不要だから。bin/loopctl と同じ host/人間レイヤ）。
...
- **`container/app/.claude/skills/loop-decompose/`** = **仕様フェーズの分解スキル（executor 側＝無人）**。`poll-spec.sh` が `LOOP_SOURCE=spec` で投函した分解タスク（本文に `spec_phase:`）を loop-task が委譲する。spec の1フェーズを 1 PR 単位の issue 群に割って起票するだけ（コード・PR は作らない）。loop-issue（人間用）と違い **`setup-target` が配布する**（bot の `/work/repo` で動くため）。
- **`planner/commands/loop-issue.md`**（root の人間レイヤ。コンテナには入れない＝`container/app/` の外）= **人間が対話的に叩く起票スラッシュコマンド** `/loop-issue`（`loop-spec` の下流＝あちらは spec 全体、こちらは 1 issue＝1 PR）。`loop-task`（executor＝無人）と対になる入力側の道具で、`grill-me` 風に**容赦なく問い詰めて**曖昧さを潰し、対象 repo に接地した精密な issue を `loop`/`loop:proposed` で起票する。**`setup-target` では配布しない**（bot の `/work/repo` ではなく、人間が対象 repo を checkout したセッションで使う）。設置は人間のグローバル `~/.claude/commands/` か、対象 repo へコミット。狙いは「issue の質がループ出力の質を決める」を踏まえること（認知の欠落(1)の人間ドリブン版）。skill ではなく command にしたのは「人間が意図的に起動する対話ツール」だから。
- **`planner/commands/loop-spec.md`**（root の人間レイヤ。コンテナには入れない）= **人間が対話的に叩く spec 起草スラッシュコマンド** `/loop-spec`（`loop-issue` の上流）。`grill-me` 風に**容赦なく問い詰めて**曖昧さを潰し、対象 repo に接地して `poll-spec` が食う `spec/`（`00-overview.md` ＋ フェーズ毎の `NN-slug.md`）を書き出す。issue までは割らない（それは `loop-decompose` の仕事）。**ファイル生成のみ**で commit/push/PR はしない（spec は既定ブランチに入った瞬間に自動分解が始まる重い変更なので人間が反映を判断）。skill ではなく command にしたのは「人間が意図的に起動する対話ツール」だから。`loop-issue` 同様 **`setup-target` では配布しない**（人間が `~/.claude/commands/` か対象 repo にコピーして使う）。
