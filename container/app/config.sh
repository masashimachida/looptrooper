...
# ── claude セッションへ渡さない秘密（session-keeper が tmux 起動時に環境から抜く）──
#   .env の秘密は env_file でコンテナ全プロセスに入るが、LLM(claude) の env に残すと
#   printenv 一発で読める（settings.json の deny は Read(.env) だけ＝env は守れない）。
#   claude に必要なのは gh ラッパー経由の短命トークンだけで、App 鍵や webhook そのものは不要。
#   トークンの鮮度は鍵を持つ側（poller が毎 tick bin/gh-token.sh でキャッシュを温める）が保ち、
#   claude 側の gh-token.sh はキャッシュへのフォールバックで読むだけ（gh-token.sh 参照）。
#   ※静的 GH_TOKEN(PAT) 運用では PAT がキャッシュ経由で claude から読める＝長命秘密は
...
#   実行頻度は POLL_DEPS_CRON（下のスケジュール節）が唯一の権威。旧 DEPS_INTERVAL の内部スロットルは廃止。
...
# ── 仕様書駆動の自律分解（poll-spec.sh が spec/ からフェーズ毎に issue 群を生成）──
#   対象 repo の SPEC_DIR（既定 spec/）に 00-overview.md（全体像・共通制約）＋ NN-slug.md（フェーズ1枚）を置く。
#   フェーズ = GitHub マイルストーン「NN: slug」。前フェーズ完了（open_issues==0 かつ closed>0）を検知したら
#   次フェーズを1回だけ分解（遅延分解＝前フェーズのマージ済み実物を読んで割る）。LLM を呼ぶのは分解タスクだけ。
#   spec/ が無ければ自動的に no-op（=この機能を使う repo だけ spec/ を置けば有効。マージは人間ゲートのまま）。
export SPEC_DIR="${SPEC_DIR:-spec}"   # 対象 repo 内の仕様ディレクトリ（repo ルートからの相対）
...
export POLL_DEPS_CRON="${POLL_DEPS_CRON:-0 3 * * *}"   # 依存監査は重い割に急がないので既定で毎日3時（空にすると既定間隔=15分毎になり過剰）
export POLL_CI_CRON="${POLL_CI_CRON:-}"
export POLL_SPEC_CRON="${POLL_SPEC_CRON:-}"           # 空＝既定間隔（15分毎）。フェーズ境界は稀なので間隔チェックで十分（マイルストーン一覧を見るだけ＝安い）
