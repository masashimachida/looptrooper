...
1. **タスク読込** — 指定された `<path>`（例 `/work/loop/.loop/queue/<id>.md`）を Read。ファイル名から `<id>` を控える。本文に `issue #<N>` があれば `<N>` も控える。
   ...
   - **本文に `mode: plan` があれば、実装フローではなく後述の「プランモード」に切り替える**（コードに一切触れず、方針/設計を検討して issue にコメントで応答するだけ。worktree・実装・PR は作らない）。
...
7. **報告（最後に必ず1回だけ）** — 完了報告は **`loop-report` のみ**。他の方法で「終わった」と言わない。これがドライバへの sentinel になる。
   **`loop-report` は“最後の行動”にする＝その後に締めの要約・感想・次の説明など一切出力しない**（コマンド実行で終える）。
   報告後の余計な出力はタスク境界の `/clear` と競合し（pane が idle に戻らず文脈リセットが飲まれる）、次タスクに文脈が漏れてコスト増になる。言いたいことは `--summary`/`--next` に入れる:
...
## プランモード（本文に `mode: plan` がある場合）

人間が issue に `loop:plan` ラベルを付けたときのタスク。**「実装はまだしてほしくないが、見に来て一緒に方針を詰めたい」**状態。
通常の実装フロー（手順3〜6）は**一切行わない**。**コードに触れず**、issue 本文と全コメントを読み、方針/設計を検討して
**issue にコメントで応答する**だけ。人間が返信すればまた応答する（`loop:plan` が付いている限り何度でも往復＝議論モード）。

> **絶対禁止（このモードの核心）**: worktree 作成 / ファイルの Write・Edit / `git push` / `gh pr create` / 検証コマンドの実行。
> やってよいのは **読むこと（research サブエージェント含む）と issue へのコメント**だけ。実装の合図は「人間が `loop:plan` を外して
> `loop` に付け替える」こと（その時 driver 側が実装フローへ昇格させる）であって、bot が会話の中で勝手に実装へ進んではいけない。
...
5. **issue にコメントで応答**する。最新の人間コメントに答えつつ、方針/設計・選択肢とトレードオフ・残る論点を人間可読でまとめる。
   末尾に awaiting マーカー（`<!-- loop:awaiting-reply -->`）を**必ず**入れる（poll-gh が人間の返信検知に使う）:
   ```bash
   gh issue comment <N> --body "$(cat <<'MD'
   🤖 loop-bot（プランモード／まだ実装はしていません）:

   **現状の理解**: <issue とコメントから読み取った要件・前提>
   **提案する方針**: <設計・アプローチ。必要なら段取り>
   **論点と選択肢**: <決め切れていない点を案つきで（案1 … / 案2 …）>
   **想定する変更範囲**: <どのあたりに手が入るか（research の path:line を根拠に）>

   この方針でよければ `loop:plan` を外して `loop` に付け替えてください（実装に入ります）。
   調整があればコメントで教えてください（このまま議論を続けます）。
   <!-- loop:awaiting-reply -->
   MD
   )"
   ```
6. **報告**: `loop-report --task <id> --status plan --issue <N> --summary "<応答の要点1文>"`。
   driver がこれを人間の返信待ち（awaiting）に置く。**コード変更・push・PR は無し**（このモードでは成果物はコメントだけ）。
...
- `plan` — プランモード（`mode: plan`）。コードに触れず issue に方針/設計をコメントして人間の返信待ち。`--issue <N>` 必須。実装・PR はしない。
