...
# ── 重いテストの定期実行 → 失敗を issue 自動起票（poll-test.sh）──
#   毎タスクの Verifier では重すぎて回せない e2e 等を1日1回だけ走らせ、落ちたら起票する。
#   「軽いテストは毎タスクの Verifier で / 重いテストは1日1回ここで」という補完関係。
#   HEAVY_TEST_CMD が空なら丸ごと skip（＝既定オフ。使うプロジェクトだけ .env で埋める）。
#   重複抑制: 隠しマーカー付きの open issue が既にあれば再起票しない（持続失敗の量産を防ぐ）。
export HEAVY_TEST_CMD="${HEAVY_TEST_CMD:-}"             # 重いテストコマンド（例: docker compose run --rm nextapp npm run test:e2e）。空＝無効
export HEAVY_TEST_INTERVAL="${HEAVY_TEST_INTERVAL:-86400}" # 実行間隔秒（既定24h。poll-test が自己スロットル）
export HEAVY_TEST_TAIL="${HEAVY_TEST_TAIL:-80}"        # issue 本文に貼る失敗ログの末尾行数（漏えい面・肥大化を抑える）
export HEAVY_TEST_LABEL="${HEAVY_TEST_LABEL:-loop:proposed}" # 起票ラベル（既定 proposed＝人間承認後。即修正なら loop）
...
export ENABLE_POLL_TEST="${ENABLE_POLL_TEST:-true}"      # 重いテスト定期実行 → 失敗を issue 自動起票（HEAVY_TEST_CMD 未設定なら実質無効）
