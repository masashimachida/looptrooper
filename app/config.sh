...
# ── ポーラーの実行スケジュール（任意・ポーラー毎に個別指定）──────────
#   空＝従来どおり毎巡回（間隔ベース。deps/ci は中で self-throttle）。
#   "HH:MM"（または "HH"）を入れると、そのポーラーは毎日その時刻以降に1回だけ走る
#   （cron 不要。state/poll-<name>.lastday で1日1回を担保。粒度は巡回間隔＝指定時刻を過ぎた最初の周回で発火）。
#   時刻は TZ（既定 Asia/Tokyo）基準。主入力(GH/PR)は反応速度のため通常は空のまま、1日1回系(DEPS/CI)向き。
export POLL_GH_AT="${POLL_GH_AT:-}"
export POLL_PR_AT="${POLL_PR_AT:-}"
export POLL_OUTCOME_AT="${POLL_OUTCOME_AT:-}"
export POLL_DEPS_AT="${POLL_DEPS_AT:-}"
export POLL_CI_AT="${POLL_CI_AT:-}"
